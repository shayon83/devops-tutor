...
The LiveKit Agent worker SHALL process voice turns using a Socratic SRE persona with Direct Speech-to-Speech LLM inference (~350ms response latency), while supporting a configurable `.env` feature flag (`VOICE_PIPELINE_MODE=realtime|modular`) to switch to a STT -> LLM -> TTS modular pipeline.
...
The system SHALL expose Prometheus metrics for RED signals (request rate, error rate, duration), End-to-End voice turn latency, barge-in frequency, and talk-time ratios, shipping container logs to Loki and provisioning Grafana dashboards automatically on startup.
